...
const SECTION_API: Gstr1aDownloadApiName = 'b2b';
...
@Component({
  selector: 'lib-gstr1a-b2b-section-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr1a-b2b-section.page.html',
  styleUrl: './gstr1a-b2b-section.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr1aB2bSectionPageComponent {
  ...
  readonly sectionHint =
    GSTR1A_DOWNLOAD_API_OPTIONS.find((x) => x.value === 'b2b')?.description ?? 'B2B invoices';
  ...
  readonly addRecordLink = computed((): readonly string[] => {
    ...
    const r = this.retPeriod().trim();
    return ['/gstr1/workspace/gstr1-download/section', 'b2b', g, r, 'add-b2b'];
  });
  ...
  async submitGstr1aRetsave(): Promise<void> {
    ...
    const gt =
      typeof msg['gt'] === 'number' ? msg['gt'] : this.sumB2bLikeInvoiceValues(section);
    const curGt =
  ...
  downloadJson(): void {
    ...
    a.href = url;
    a.download = `gstr1a-b2b-${this.gstin()}-${this.retPeriod()}.json`;
    a.click();
