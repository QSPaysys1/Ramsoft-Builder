...
import {
  GSTR1A_AMEND_RECORD_DETAIL_TILES,
...
@Component({
  selector: 'lib-gstr1a-hub-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr1a-hub.page.html',
  styleUrl: './gstr1a-hub.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr1aHubPageComponent {
  ...
  readonly amendRecordTiles = GSTR1A_AMEND_RECORD_DETAIL_TILES;
  ...
  async navigateToAddTile(index: number): Promise<void> {
    ...
    if (index === 0 && api === 'b2b') {
      ...
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/b2b'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    }
    if (index === 1 && api === 'b2cl') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/b2cl'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 2 && api === 'exp') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/exp'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 3 && api === 'b2cs') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/b2cs'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 4 && api === 'nil') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/nil'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 5 && api === 'cdnr') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/cdnr'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 6 && api === 'cdnur') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/cdnur'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 7 && api === 'at') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/at'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
    ...
    if (index === 9 && api === 'hsnsum') {
      ...
      const fl = this.filingLabel().trim();
      if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
        await this.router.navigate(['/gstr1a/hsn'], {
          queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
        });
      }
      return;
  ...
  async navigateToAmendTile(tile: Gstr1AmendRecordDetailTile): Promise<void> {
    ...
    const fl = this.filingLabel().trim();
    if (g.length === 15 && RETURN_PERIOD_REGEX.test(r)) {
      await this.router.navigate(['/gstr1a', a], {
        queryParams: { gstin: g, ret_period: r, filing_status: fl || undefined },
      });
      return;
    }
