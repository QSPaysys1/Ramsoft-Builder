...
const SECTION_API: Gstr1aDownloadApiName = 'exp';

/** `exp[]` rows are `{ exp_typ, inv[] }` — synthesize a group label for the hierarchy parser. */
function augmentExpRecordsForHierarchy(records: unknown[]): unknown[] {
  return records.map((rec) => {
    ...
    const typ = String(b['exp_typ'] ?? '').trim().toUpperCase();
    const label =
      typ === 'WPAY'
        ? 'With payment of tax (WPAY)'
        : typ === 'WOPAY'
          ? 'Without payment of tax (WOPAY)'
          : typ
            ? `Exports (${typ})`
            : 'Exports';
    return { ...b, ctin: label };
...
@Component({
  selector: 'lib-gstr1a-exp-section-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr1a-exp-section.page.html',
  styleUrl: './gstr1a-exp-section.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr1aExpSectionPageComponent {
  ...
  readonly sectionHint =
    GSTR1A_DOWNLOAD_API_OPTIONS.find((x) => x.value === 'exp')?.description ??
    'Export invoices';
  ...
  readonly addRecordLink = computed((): readonly string[] => {
    ...
    const r = this.retPeriod().trim();
    return ['/gstr1/workspace/gstr1-download/section', 'exp', g, r, 'add-exp'];
  });
  ...
  async loadSection(): Promise<void> {
    ...
    try {
      ...
      const augmented = augmentExpRecordsForHierarchy(bucket);
  ...
  async submitGstr1aRetsave(): Promise<void> {
    ...
    const gt =
      typeof msg['gt'] === 'number' ? msg['gt'] : this.sumExpInvoiceValues(section);
    const curGt =
  ...
  private sumExpInvoiceValues(section: unknown): number {
    if (!Array.isArray(section)) {
      return 0;
    }
    let s = 0;
    for (const grp of section) {
      if (grp && typeof grp === 'object' && Array.isArray((grp as Record<string, unknown>)['inv'])) {
        for (const inv of (grp as Record<string, unknown>)['inv'] as unknown[]) {
  ...
  downloadJson(): void {
    ...
    a.href = url;
    a.download = `gstr1a-exp-${this.gstin()}-${this.retPeriod()}.json`;
    a.click();
