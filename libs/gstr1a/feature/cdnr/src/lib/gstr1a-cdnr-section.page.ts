...
const SECTION_API: Gstr1aDownloadApiName = 'cdnr';

/** CDNR buckets use `nt[]`; hierarchy parser expects `inv[]`. Skip empty note lists. */
function normalizeCdnrRecordsForHierarchy(records: unknown[]): unknown[] {
  const out: unknown[] = [];
...
@Component({
  selector: 'lib-gstr1a-cdnr-section-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr1a-cdnr-section.page.html',
  styleUrl: './gstr1a-cdnr-section.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr1aCdnrSectionPageComponent {
  ...
  readonly sectionHint =
    GSTR1A_DOWNLOAD_API_OPTIONS.find((x) => x.value === 'cdnr')?.description ??
  ...
  readonly recipientRows = computed(() => {
    const rows: {
      ctin: string;
      noteCount: number;
      valSum: number;
  ...
  readonly addRecordLink = computed((): readonly string[] => {
    ...
    const r = this.retPeriod().trim();
    return ['/gstr1/workspace/gstr1-download/section', SECTION_API, g, r, 'add-cdnr'];
  });
  ...
  ctinExpanded(ctin: string): boolean {
    return this.expandedCtins().has(ctin);
  }
  ...
  async loadSection(): Promise<void> {
    ...
    try {
      ...
      const normalized = normalizeCdnrRecordsForHierarchy(bucket);
  ...
  async submitGstr1aRetsave(): Promise<void> {
    ...
    const gt =
      typeof msg['gt'] === 'number' ? msg['gt'] : this.sumCdnrNoteValues(section);
  ...
  private sumCdnrNoteValues(section: unknown): number {
    ...
    for (const g of section) {
      ...
      const nt = (g as Record<string, unknown>)['nt'];
      if (!Array.isArray(nt)) {
        continue;
      }
      for (const n of nt) {
  ...
  downloadJson(): void {
    ...
    a.href = url;
    a.download = `gstr1a-cdnr-${this.gstin()}-${this.retPeriod()}.json`;
    a.click();
