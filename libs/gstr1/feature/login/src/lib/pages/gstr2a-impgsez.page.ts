...
import {
  ...
  gstr2aImpgsezRowKey,
  gstr2aImpgsezRowsToCsv,
  gstr2ImpgsezLogicalError,
  parseGstr2ImpgsezFromPayload,
  RETURN_PERIOD_REGEX,
  type Gstr2aImpgsezRow,
...
export interface Gstr2aImpgsezColumnDef {
  readonly id: string;
...
export const GSTR2A_IMPGSEZ_TABLE_COLUMNS: readonly Gstr2aImpgsezColumnDef[] = [
  {
  ...
  {
    id: 'sezGstin',
    label: 'GSTIN of SEZ supplier',
    field: 'sezGstin',
    highlight: true,
  },
  { id: 'tradeName', label: 'Trade name', field: 'tradeName' },
...
function defaultColumnVisibility(): Record<string, boolean> {
  ...
  for (const col of GSTR2A_IMPGSEZ_TABLE_COLUMNS) {
    out[col.id] = true;
  }
  return out;
}
...
@Component({
  selector: 'lib-gstr2a-impgsez-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr2a-impgsez.page.html',
  styleUrl: './gstr2a-impgsez.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr2aImpgsezPageComponent {
  ...
  readonly tableColumns = GSTR2A_IMPGSEZ_TABLE_COLUMNS;
  readonly pageTitle =
  ...
  readonly importRows = signal<readonly Gstr2aImpgsezRow[]>([]);
  ...
  readonly filteredRows = computed(() => {
    ...
    return rows.filter(
      (row) =>
        ...
        row.sezGstin.toLowerCase().includes(q) ||
  ...
  isHighlightedField(field: keyof Gstr2aImpgsezRow): boolean {
    return !!this.tableColumns.find((c) => c.field === field)?.highlight;
  }

  trackRow(row: Gstr2aImpgsezRow): string {
    return gstr2aImpgsezRowKey(row);
  }
  ...
  cellValue(row: Gstr2aImpgsezRow, field: Gstr2aImpgsezColumnDef['field']): string {
  ...
  async loadImports(): Promise<void> {
    ...
    try {
      const payload = await firstValueFrom(
        this.api.fetchGstr2Impgsez({ gstin, ret_period }),
      );
      const topErr = gstr2ImpgsezLogicalError(payload);
      if (topErr) {
        this.logicalError.set(topErr);
        this.viewState.set('error');
        return;
      }
      const rows = parseGstr2ImpgsezFromPayload(payload);
  ...
  downloadCsv(): void {
    ...
    const csv = gstr2aImpgsezRowsToCsv(
      rows,
    ...
    a.href = url;
    a.download = `gstr2a-impgsez-${this.gstin()}-${this.retPeriod()}.csv`;
    a.click();
