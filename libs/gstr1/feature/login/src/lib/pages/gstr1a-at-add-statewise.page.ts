...
import { Gstr1GstnOtpApiService, RETURN_PERIOD_REGEX } from '@ramsoft-builder/gstr1/data-access/gstzen-auth';
import { firstValueFrom, merge } from 'rxjs';
import { startWith } from 'rxjs/operators';
import { INDIAN_STATE_POS_OPTIONS } from '../constants/indian-state-pos.options';

/** Portal “Applicable % of Tax Rate” → NIC `diff_percent` (fraction). */
const GSTR1_ADVANCE_DIFF_PERCENT_OPTIONS = [
  { label: '65%', value: '0.65' },
  { label: '70%', value: '0.70' },
  { label: '75%', value: '0.75' },
  { label: '80%', value: '0.80' },
...
@Component({
  selector: 'lib-gstr1a-at-add-statewise-page',
  standalone: true,
  imports: [JsonPipe, RouterLink, ReactiveFormsModule],
  templateUrl: './gstr1a-at-add-statewise.page.html',
  styleUrl: './gstr1-b2b-add-record.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr1aAtAddStatewisePageComponent {
  ...
  readonly rateSlabs = AT_RATE_SLABS;
  readonly diffPercentOptions = GSTR1_ADVANCE_DIFF_PERCENT_OPTIONS;
  ...
  paramsValid(): boolean {
    return this.filerGstin().length === 15 && RETURN_PERIOD_REGEX.test(this.retPeriod().trim());
  }

  backUrl(): unknown[] {
    return ['/gstr1/workspace/gstr1a-at', this.filerGstin(), this.retPeriod().trim()];
  }
  ...
  /** Restore GSTR-1A workspace (tile grid) when user follows breadcrumbs. */
  workspaceGstr1aQueryParams(): Record<string, string> {
    const o: Record<string, string> = {
  ...
  private buildRetsavePayload(): Record<string, unknown> | null {
    ...
    for (let i = 0; i < AT_RATE_SLABS.length; i++) {
      const row = rowVals[i];
      const rt = AT_RATE_SLABS[i];
  ...
  async submit(): Promise<void> {
    ...
    if (!this.rateGridHasData()) {
      this.saveError.set({
        message:
          'Enter gross advance received and tax in at least one rate row under Item details.',
      });
      this.cdr.markForCheck();
      return;
    }
