...
import {
  ...
  gstr2aEcoRowsToCsv,
  gstr2EcomLogicalError,
  parseGstr2EcomOperatorsFromPayload,
  RETURN_PERIOD_REGEX,
  type Gstr2aEcoOperatorRow,
...
export interface Gstr2aEcoColumnDef {
  readonly id: string;
...
export const GSTR2A_ECO_TABLE_COLUMNS: readonly Gstr2aEcoColumnDef[] = [
...
function defaultColumnVisibility(): Record<string, boolean> {
  ...
  for (const col of GSTR2A_ECO_TABLE_COLUMNS) {
    out[col.id] = true;
  }
  return out;
}
...
@Component({
  selector: 'lib-gstr2a-eco-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr2a-eco.page.html',
  styleUrl: './gstr2a-eco.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr2aEcoPageComponent {
  ...
  readonly tableColumns = GSTR2A_ECO_TABLE_COLUMNS;
  readonly viewState = signal<ViewState>('idle');
  readonly operatorRows = signal<readonly Gstr2aEcoOperatorRow[]>([]);
  ...
  readonly visibleColumns = computed(() =>
    GSTR2A_ECO_TABLE_COLUMNS.filter((col) => this.isColumnVisible(col.id)),
  );
  ...
  constructor() {
    ...
    afterNextRender(() => {
      ...
      void this.loadEcoDocuments();
    });
  }

  trackRow(row: Gstr2aEcoOperatorRow): string {
  ...
  isColumnVisible(columnId: string): boolean {
    const col = GSTR2A_ECO_TABLE_COLUMNS.find((c) => c.id === columnId);
    if (col?.locked) {
      return true;
    }
  ...
  cellValue(row: Gstr2aEcoOperatorRow, field: keyof Gstr2aEcoOperatorRow): string {
  ...
  async loadEcoDocuments(): Promise<void> {
    ...
    try {
      const payload = await firstValueFrom(
        this.api.fetchGstr2Ecom({ gstin, ret_period }),
      );
      const topErr = gstr2EcomLogicalError(payload);
      if (topErr) {
        this.logicalError.set(topErr);
        this.viewState.set('error');
        return;
      }
      const rows = parseGstr2EcomOperatorsFromPayload(payload);
  ...
  downloadCsv(): void {
    ...
    a.href = url;
    a.download = `gstr2a-eco-${this.gstin()}-${this.retPeriod()}.csv`;
    a.click();
