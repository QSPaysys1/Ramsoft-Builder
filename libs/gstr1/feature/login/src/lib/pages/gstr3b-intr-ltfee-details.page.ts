...
function normalizeDraftIntrLtfee(draft: Gstr3bIntrLtfee): Gstr3bIntrLtfee {
  return {
    intr_details: { ...draft.intr_details },
    ltfee_details: {
      iamt: 0,
      camt: draft.ltfee_details.camt,
      samt: draft.ltfee_details.samt,
      csamt: 0,
    },
  };
}
...
export class Gstr3bIntrLtfeeDetailsPageComponent {
  ...
  readonly draftIntrLtfee = signal<Gstr3bIntrLtfee>(emptyGstr3bIntrLtfee());
  ...
  touchIntrLtfee(): void {
    const draft = normalizeDraftIntrLtfee(structuredClone(this.draftIntrLtfee()));
    this.draftIntrLtfee.set(draft);
  ...
  cancel(): void {
    this.draftIntrLtfee.set(structuredClone(this.retsaveForm().intr_ltfee));
    ...
    try {
      ...
      if (!retsumErr) {
        ...
        if (form) {
          ...
          this.draftIntrLtfee.set(normalizeDraftIntrLtfee(structuredClone(form.intr_ltfee)));
