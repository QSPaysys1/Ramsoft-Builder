...
import {
  ...
  gstr2aEcomaRowsToCsv,
  gstr2EcomaLogicalError,
  parseGstr2EcomaOperatorsFromPayload,
  RETURN_PERIOD_REGEX,
  type Gstr2aEcomaOperatorRow,
...
export interface Gstr2aEcomaColumnDef {
  readonly id: string;
...
export const GSTR2A_ECOA_TABLE_COLUMNS: readonly Gstr2aEcomaColumnDef[] = [
  { id: 'ecoGstin', label: 'GSTIN of ECO', field: 'ecoGstin', locked: true },
  { id: 'ecoName', label: 'Name of ECO', field: 'ecoName' },
  { id: 'documentSection', label: 'Section', field: 'documentSection' },
  { id: 'documentCount', label: 'No. of documents', field: 'documentCount' },
  {
...
function defaultColumnVisibility(): Record<string, boolean> {
  ...
  for (const col of GSTR2A_ECOA_TABLE_COLUMNS) {
    out[col.id] = true;
  }
  return out;
}
...
@Component({
  selector: 'lib-gstr2a-ecoa-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr2a-ecoa.page.html',
  styleUrl: './gstr2a-ecoa.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr2aEcomaPageComponent {
  ...
  readonly tableColumns = GSTR2A_ECOA_TABLE_COLUMNS;
  readonly viewState = signal<ViewState>('idle');
  readonly operatorRows = signal<readonly Gstr2aEcomaOperatorRow[]>([]);
  ...
  readonly filteredRows = computed(() => {
    ...
    return rows.filter(
      (row) =>
        row.ecoGstin.toLowerCase().includes(q) ||
        row.ecoName.toLowerCase().includes(q) ||
        row.documentSection.toLowerCase().includes(q),
    );
  });
  ...
  readonly visibleColumns = computed(() =>
    GSTR2A_ECOA_TABLE_COLUMNS.filter((col) => this.isColumnVisible(col.id)),
  );
  ...
  constructor() {
    ...
    afterNextRender(() => {
      ...
      void this.loadEcomaDocuments();
    });
  }

  trackRow(row: Gstr2aEcomaOperatorRow): string {
    return `${row.ecoGstin}::${row.documentSection}`;
  }
  ...
  isColumnVisible(columnId: string): boolean {
    const col = GSTR2A_ECOA_TABLE_COLUMNS.find((c) => c.id === columnId);
    if (col?.locked) {
      return true;
    }
  ...
  cellValue(row: Gstr2aEcomaOperatorRow, field: keyof Gstr2aEcomaOperatorRow): string {
  ...
  async loadEcomaDocuments(): Promise<void> {
    ...
    this.operatorRows.set([]);

    try {
      const payload = await firstValueFrom(
        this.api.fetchGstr2Ecoma({ gstin, ret_period }),
      );
      const topErr = gstr2EcomaLogicalError(payload);
      if (topErr) {
        this.logicalError.set(topErr);
        this.viewState.set('error');
        return;
      }
      const rows = parseGstr2EcomaOperatorsFromPayload(payload);
      this.operatorRows.set(rows);
  ...
  downloadCsv(): void {
    ...
    const csv = gstr2aEcomaRowsToCsv(
      rows,
    ...
    a.href = url;
    a.download = `gstr2a-ecoa-${this.gstin()}-${this.retPeriod()}.csv`;
    a.click();
