...
import {
  ...
  gstr2aB2baRowsToCsv,
  gstr2B2baLogicalError,
  parseGstr2B2baSuppliersFromPayload,
  RETURN_PERIOD_REGEX,
  type Gstr2aB2baSupplierRow,
} from '@ramsoft-builder/gstr1/data-access/gstzen-auth';
import { catchError, firstValueFrom, of, switchMap } from 'rxjs';

type ViewState = 'idle' | 'loading' | 'success' | 'empty' | 'error';

export interface Gstr2aB2baColumnDef {
  readonly id: string;
...
export const GSTR2A_B2BA_TABLE_COLUMNS: readonly Gstr2aB2baColumnDef[] = [
...
function defaultColumnVisibility(): Record<string, boolean> {
  ...
  for (const col of GSTR2A_B2BA_TABLE_COLUMNS) {
    out[col.id] = true;
  }
  return out;
}
...
@Component({
  selector: 'lib-gstr2a-b2ba-page',
  standalone: true,
  imports: [JsonPipe, RouterLink],
  templateUrl: './gstr2a-b2ba.page.html',
  styleUrl: './gstr2a-b2ba.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr2aB2baPageComponent {
  ...
  readonly tableColumns = GSTR2A_B2BA_TABLE_COLUMNS;

  readonly viewState = signal<ViewState>('idle');
  readonly supplierRows = signal<readonly Gstr2aB2baSupplierRow[]>([]);
  ...
  readonly visibleColumns = computed(() =>
    GSTR2A_B2BA_TABLE_COLUMNS.filter((col) => this.isColumnVisible(col.id)),
  );
  ...
  isColumnVisible(columnId: string): boolean {
    const col = GSTR2A_B2BA_TABLE_COLUMNS.find((c) => c.id === columnId);
    if (col?.locked) {
      return true;
    }
  ...
  cellValue(row: Gstr2aB2baSupplierRow, field: keyof Gstr2aB2baSupplierRow): string {
  ...
  async loadSuppliers(): Promise<void> {
    ...
    try {
      const payload = await firstValueFrom(
        this.api.fetchGstr2B2ba({ gstin, ret_period }),
      );
      const topErr = gstr2B2baLogicalError(payload);
      if (topErr) {
        this.logicalError.set(topErr);
        this.viewState.set('error');
        return;
      }
      const rows = parseGstr2B2baSuppliersFromPayload(payload);
  ...
  downloadCsv(): void {
    ...
    const csv = gstr2aB2baRowsToCsv(rows, this.visibleColumns());
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `gstr2a-b2ba-${this.gstin()}-${this.retPeriod()}.csv`;
    a.click();
