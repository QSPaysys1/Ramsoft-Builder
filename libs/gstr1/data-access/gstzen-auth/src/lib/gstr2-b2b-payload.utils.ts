import type { Gstr2aB2bSupplierRow } from './gstr2-b2b.models';
...
function extractB2bArrayFromMessage(msg: Record<string, unknown>): unknown[] {
  const direct = msg['b2b'];
...
function extractB2bRawArray(payload: unknown): unknown[] {
  ...
  if (msg) {
    const fromMsg = extractB2bArrayFromMessage(msg);
    if (fromMsg.length > 0) {
      return fromMsg;
    }
  }
  for (const k of ['b2b', 'B2B', 'data', 'Data'] as const) {
    const v = root[k];
    if (Array.isArray(v)) {
      return v;
    }
...
function mapSupplierRow(raw: Record<string, unknown>): Gstr2aB2bSupplierRow {
  return {
...
/**
 * Normalizes GSTZen `POST gstr2/b2b/` into supplier-level rows (one per CTIN when nested).
 */
export function parseGstr2B2bSuppliersFromPayload(payload: unknown): Gstr2aB2bSupplierRow[] {
  const raw = extractB2bRawArray(payload);
  const byGstin = new Map<string, Gstr2aB2bSupplierRow>();
...
export function isGstr2B2bSuccessEnvelope(
  raw: unknown,
...
export function gstr2B2bLogicalError(payload: unknown): string | null {
  if (isGstr2B2bSuccessEnvelope(payload)) {
    return null;
  }
  const r = asRecord(payload);
  if (!r) {
    return 'Unexpected response from GSTR-2 B2B API.';
  }
  const err =
    r['error'] ??
    r['Error'] ??
  ...
  if (!gstr2StatusIndicatesSuccess(payload)) {
    return 'GSTR-2 B2B request did not return a success status.';
  }
  return null;
}

export interface Gstr2aB2bCsvColumn {
  readonly label: string;
  readonly field: keyof Gstr2aB2bSupplierRow;
}

const GSTR2A_B2B_CSV_COLUMNS: readonly Gstr2aB2bCsvColumn[] = [
...
export function gstr2aB2bRowsToCsv(
  rows: readonly Gstr2aB2bSupplierRow[],
  columns: readonly Gstr2aB2bCsvColumn[] = GSTR2A_B2B_CSV_COLUMNS,
): string {
