...
/** Normalizes GSTZen `POST gstr2/b2b/` into supplier-level rows. */
export function parseGstr2aB2bSuppliersFromPayload(
  payload: unknown,
): Gstr2aB2bSupplierRow[] {
  ...
  for (const item of raw) {
    ...
    if (Array.isArray(inv) && inv.length > 0) {
      ...
      if (parentCtin.length === 15) {
        const mapped = mapSupplierRow({ ...rec, ctin: parentCtin });
...
export function isGstr2aB2bSuccessEnvelope(
  raw: unknown,
...
export function gstr2aB2bLogicalError(payload: unknown): string | null {
  if (isGstr2aB2bSuccessEnvelope(payload)) {
    return null;
  }
