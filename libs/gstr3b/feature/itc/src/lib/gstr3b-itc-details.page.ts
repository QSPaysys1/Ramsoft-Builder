...
const ITC_AVL_LABELS: Record<string, string> = {
...
const ITC_REV_LABELS: Record<string, string> = {
  RUL: '(1) As per rules 38, 42 & 43 of CGST Rules and section 17(5)',
  OTH: '(2) Others',
};

const ITC_INELG_LABELS: Record<string, string> = {
  RUL: '(1) ITC reclaimed which was reversed under Table 4(B)(2) in earlier tax period',
  OTH: '(2) Ineligible ITC under section 16(4) & ITC restricted due to PoS rules',
};
...
@Component({
  selector: 'lib-gstr3b-itc-details-page',
  standalone: true,
  imports: [DecimalPipe, JsonPipe, RouterLink, FormsModule],
  templateUrl: './gstr3b-itc-details.page.html',
  styleUrl: './gstr3b-sup-details.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'block min-h-[60vh]' },
})
export class Gstr3bItcDetailsPageComponent {
  private readonly platformId = inject(PLATFORM_ID);
  private readonly destroyRef = inject(DestroyRef);
  private readonly route = inject(ActivatedRoute);
  private readonly api = inject(Gstr3bApiService);

  readonly itcAvlLabels = ITC_AVL_LABELS;
  readonly itcRevLabels = ITC_REV_LABELS;
  readonly itcInelgLabels = ITC_INELG_LABELS;
  readonly itcAvlTypes = GSTR3B_ITC_AVL_TYPES;
  readonly itcRevTypes = GSTR3B_ITC_REV_TYPES;
  readonly itcInelgTypes = GSTR3B_ITC_INELG_TYPES;
  ...
  readonly draftItcElg = signal<Gstr3bItcElg>(emptyGstr3bItcElg());
  ...
  isImportItcRow(ty: string): boolean {
    return ty === 'IMPG' || ty === 'IMPS';
  }

  touchItcElg(): void {
    const draft = structuredClone(this.draftItcElg());
    this.draftItcElg.set(draft);
  ...
  cancel(): void {
    this.draftItcElg.set(structuredClone(this.retsaveForm().itc_elg));
    ...
    try {
      ...
      if (!retsumErr) {
        ...
        if (form) {
          ...
          this.draftItcElg.set(structuredClone(form.itc_elg));
